...
def _interest(amount, days, annual_rate):
    return round(amount * (annual_rate / 100.0) * (days / 360.0), 2)


def seed_if_empty():
    """Populate demo data when the database is empty.

    Must be called after db.init_db().
    """
    conn = db.get_db()
    try:
    ...
    for key, name in banks:
        with db.get_db() as conn_bank:
            db.upsert_bank(conn_bank, key, name)
    ...
    cl_ids = []
    with db.get_db() as conn_cl:
        for payload in credit_lines:
            cl_ids.append(db.create_credit_line(conn_cl, payload))
