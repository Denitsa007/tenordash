...
def export_xlsx(export_path=None):
    ...
    Args:
        export_path: Optional directory override. If None, reads from DB settings
                     then falls back to config.EXPORT_PATH.
    """
    conn = db.get_db()
    try:
    ...
    export_file = os.path.join(export_dir, "tenordash.xlsx")
